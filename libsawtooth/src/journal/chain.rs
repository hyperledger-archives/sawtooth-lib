...
impl<TEP: ExecutionPlatform + Clone + 'static> ChainController<TEP> {
    ...
    fn set_genesis(
        ...
        if block.header().previous_block_id() == NULL_BLOCK_IDENTIFIER {
                ...
                .unwrap_or(false)
            {
                ...
                guard.notify_on_chain_updated(block.block().clone(), vec![], vec![]);
            }
        }
    ...
    fn handle_block_commit(&mut self, block: &BlockPair) -> Result<(), ChainControllerError> {
        {
            ...
            loop {
                ...
                let new_roots: Vec<String> = result
                    .new_chain
                    .iter()
                    .map(|block| block.header().state_root_hash().to_string())
                    .collect();
                let current_roots: Vec<(u64, String)> = result
                    ...
                    .map(|block| {
                        (
                            ...
                            block.header().state_root_hash().to_string(),
                        )
                    })
                    .collect();
    ...
    fn initialize_chain_head(&mut self) {
        ...
        if let Some(chain_head) = chain_head {
            ...
            guard.notify_on_chain_updated(chain_head.block().clone(), vec![], vec![]);
        }
    }
